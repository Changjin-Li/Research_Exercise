...
class LoopDetector:
    ...
    def get_image_path(self):
        ...
        for extension in image_extensions:
            image_paths.extend(list(Path(self.image_dir).glob(f"*{extension}")))
            image_paths.extend(list(Path(self.image_dir).glob(f"*{extension.upper()}")))
    ...
    def extract_descriptors(self):
        ...
        transform = self._input_transform(image_size=self.image_size)
        descriptors = []
        ...
        for i in tqdm(range(0, len(self.image_paths), self.batch_size), desc="Extracting features"):
            ...
            for path in batch_paths:
                try:
                    ...
                    image = torch.zeros(3, 224, 224) if self.image_size is None else torch.zeros(3, self.image_size[0], self.image_size[1])
                    batch_images.append(image)
    ...
    def _apply_nms_filter(self, loop_closures, nms_threshold):
        ...
        filtered_loops = []
        suppressed = set()
        max_frame = max(max(index1, index2) for index1, index2, _ in loop_closures)

        for index1, index2, sim in sorted_loops:
            if index1 in suppressed or index2 in suppressed:
                continue

            filtered_loops.append((index1, index2, sim))
            suppressed_range = set()

            start1 = max(0, index1 - nms_threshold)
            end1 = min(index1 + nms_threshold + 1, index2)
            suppressed_range.update(range(start1, end1))

            start2 = max(index1 + 1, index2 - nms_threshold)
            end2 = min(index2 + nms_threshold + 1, max_frame + 1)
            suppressed_range.update(range(start2, end2))

            suppressed.update(suppressed_range)

        return filtered_loops

    def _ensure_decenting_order(self, tuples_list):
    ...
    def find_loop_closures(self):
        ...
        for i in range(len(self.descriptors)):
            for j in range(1, self.top_k + 1): # Skip first result (self)
                neighbor_index = indices[i, j]
                similarity = similarities[i, j]
                if similarity > self.similarity_threshold and abs(i - neighbor_index) > 10:
                    if i < neighbor_index:
                        loop_closures.append((i, neighbor_index, similarity))
                    else:
                        loop_closures.append((neighbor_index, i, similarity))
        ...
        self.loop_closures = self._ensure_decenting_order(loop_closures)
    ...
    def get_loop_list(self):
        return [(index1, index2) for index1, index2, _ in self.loop_closures]
