...
class Sim3LoopOptimizer:
    ...
    def numpy_to_pypose_sim3(self, s: float, R_mat: np.ndarray, t_vec: np.ndarray) -> pp.Sim3:
        ...
        data = np.concatenate([t_vec, q, np.array([s])]) # R^8: [t(3), q(4), s(1)]
    ...
    def pypose_sim3_to_numpy(self, sim3: pp.Sim3) -> Tuple[float, np.ndarray, np.ndarray]:
        ...
        t, q, s = data[:3], data[3:7], data[7]
        R_mat = R.from_quat(q).as_matrix()
        return s, R_mat, t

    def sequential_to_absolute_poses(self, sequential_transforms: List[Tuple[float, np.ndarray, np.ndarray]]) -> Tensor:
        """
        ...
        poses.append(identity) # The first frame
    ...
    def SE3_to_Sim3(self, x: Tensor) -> pp.Sim3:
        """Convert SE3 to Sim3 (add unit scale)"""
        ones = torch.ones_like(x[..., :1])
        out = torch.cat((x, ones), dim=-1)
        return pp.Sim3(out)

    def build_loop_constraints(self, loop_constraints: List[Tuple[int, int, Tuple[float, np.ndarray, np.ndarray]]]) -> Tuple[Tensor, Tensor, Tensor]:
    ...
    def residual(self, Ginv, input_poses, dSloop, ii, jj, jacobian=False):
        ...
        return r, (J_Ginv_i, J_Ginv_j, iii, jjj)
    ...
    def optimize(
        ...
        for itera in range(max_iterations):
            r, (J_Ginv_i, J_Ginv_j, iii, jjj) = self.residual(Ginv, input_poses, dSloop, ii_loop, jj_loop, jacobian=True)
            if r.numel() == 0:
                print("No residuals to optimize.")
                break

            current_cost = r.square().mean().item()
            residual_history.append(current_cost)
            try: # Solve linear system
                begin_time = time.time()
                if self.solve_system_version == 'cpp':
                    delta_pose = sim3solve.solve_system(J_Ginv_i, J_Ginv_j, iii, jjj, r, 0.0, lambda_init, -1)
                elif self.solve_system_version == 'python':
                    delta_pose = solve_system_py(J_Ginv_i, J_Ginv_j, iii, jjj, r, 0.0, lambda_init, -1)
                else:
                    delta_pose = 0.
                    print(f"Solver version has not been chosen! ('python' or 'cpp')")
                end_time = time.time()
            except Exception as e:
                print(f"Solver failed at iteration {itera}: {e}")
                break

            Ginv_tmp = Ginv + delta_pose
            new_r = self.residual(Ginv_tmp, input_poses, dSloop, ii_loop, jj_loop)
            new_cost = new_r.square().mean().item() if new_r.numel() > 0 else float('inf')

            # L-M
            if new_cost < current_cost:
                Ginv = Ginv_tmp
                lambda_init = lambda_init / 2
                print(f"Iteration {itera}: cost {current_cost:.14f} -> {new_cost:.14f} (accepted)", end=' | ')
            else:
                lambda_init = lambda_init * 2
                print(f"Iteration {itera}: cost {current_cost:.14f} -> {new_cost:.14f} (rej)     ", end=' | ')
            print(f'Time of solver ({self.solve_system_version}): {(end_time - begin_time) * 1000:.4f} ms')

            if current_cost < 1e-5 and itera >= 4:
                if len(residual_history) >= 5:
                    improvement_ratio = residual_history[-5] / residual_history[-1]
                    if improvement_ratio < 1.5:
                        print(f"Converged at iteration {itera}")
                        break
